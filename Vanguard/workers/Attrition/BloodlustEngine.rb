...
The regular expressions use [PCRE] as the engine.PCRE - Perl Compatible Regular Expressions
...
This article focuses on Audit as a control that must be built into most any application with significant impact for security or privacy. Various other audit processes are performed to promote software security and other organizational goals. But these are beyond the scope of the present article, which is focused on the support for functions required to monitor uses of software.

In addition to the establishment of policies and procedures and other administrative processes related to software audit, U.S. National Institute of Standards and Technology (NIST) Special Publication 800-53 lists nine suggested audit controls that would be directly implemented within software as a feature:
